...
const Dashboard = () => {
  ...
  const drawCharts = () => {
    ...
    const color = d3.scaleOrdinal().domain(pieData.map(d => d.label)).range(['#4caf50', '#f44336']);
    ...
    const x = d3.scaleBand().domain(catData.map(d => d.category)).range([0, width]).padding(0.2);
    const y = d3.scaleLinear().domain([0, d3.max(catData, d => d.total)]).nice().range([height - 30, 0]);

    catSvg.append('g').attr('transform', `translate(0,${height - 30})`).call(d3.axisBottom(x));
    catSvg.append('g').attr('transform', 'translate(30,0)').call(d3.axisLeft(y));
          ...
          <div className="summary-cards">
            <div className="summary-card income">
              ...
              <div className="card-content">
                <h3>Total Income</h3>
                <p className="card-value">{summary.total_income}</p>
              </div>
            </div>
            <div className="summary-card expense">
              ...
              <div className="card-content">
                <h3>Total Expense</h3>
                <p className="card-value">{summary.total_expense}</p>
              </div>
            </div>
            <div className={`summary-card balance ${summary.balance >= 0 ? 'positive' : 'negative'}`}>
              ...
              <div className="card-content">
                <h3>Balance</h3>
                <p className="card-value">{summary.balance}</p>
              </div>
            </div>
          </div>
